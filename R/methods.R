...
print.bestSubset <- function(x, ...) {
    ...
    if (
        !is.null(info$n_observations_original) &&
            info$n_observations_original != info$n_observations
    ) {
        cat(
            " ",
            format_model_summary_line(
                info$n_observations_original,
                info$n_observations
            ),
            "\n"
        )
    ...
    cat(
        "  Max variables:",
        if (info$max_variables == info$n_predictors) {
            "all"
        } else {
            info$max_variables
        },
    ...
    if (info$use_cv) {
        cat(
            ...
            info$cv_folds,
            "-fold",
            if (info$cv_repeats > 1) paste(",", info$cv_repeats, "repeats"),
            ")",
            sep = ""
        )
    }
...
summary.bestSubset <- function(object, ...) {
    ...
    coef_names <- create_coefficient_names(
        best$variables,
        object$call_info$include_intercept
    )
    ...
    if (!is.null(object$na_info) && object$na_info$n_removed > 0) {
        cat(paste(
            ...
            object$na_info$effective_n,
            "complete observations out of",
            object$na_info$original_n,
            "total observations.\n"
        ))
        cat(paste(
            object$na_info$n_removed,
...
predict.bestSubset <- function(
    ...
    if (ncol(newdata) < max(object$best_model$variables)) {
        stop(
            "newdata must have at least ",
            max(object$best_model$variables),
            " columns"
        )
    }
    ...
    if (
        !is.null(object$na_info) &&
            object$na_info$na_action_used %in% c("na.omit", "na.exclude")
    ) {
        ...
        if (na_check$has_missing) {
            if (object$na_info$na_action_used == "na.exclude") {
                ...
                predictions_clean <- predict_best_subset(
                    object$best_model,
                    newdata_clean,
                    type
                )
                ...
                warning(
                    "newdata contains missing values. Using complete cases only.",
                    call. = FALSE
                )
                complete_cases <- na_check$complete_cases
        ...
        !is.null(object$na_info) && object$na_info$na_action_used == "na.fail"
    ) {
        # Check for any missing values and fail if found
        if (any(is.na(newdata))) {
            stop(
                "newdata contains missing values. Model was fitted with na.action = na.fail"
            )
        }
    }
